...
exports.insertFavouriteByUserId = async (
  user_id,
  { imdb_id, movie_poster, movie_title }
) => {
  if (!user_id || !imdb_id || !movie_title)
    return Promise.reject({ status: 400, msg: "missing required fields" });

  const {
    rows: [row],
  } = await db.query(
    "INSERT INTO favourites (user_id, imdb_id, movie_poster, movie_title) VALUES ($1, $2, $3, $4) RETURNING *;",
    [user_id, imdb_id, movie_poster, movie_title]
  );
